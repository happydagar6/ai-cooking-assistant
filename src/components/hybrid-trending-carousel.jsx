...
import { Flame, Globe, Clock, ChefHat } from 'lucide-react';
import { useHybridTrending } from '@/hooks/use-hybrid-recommendations';
import { Button } from '@/components/ui/button';

/**
 * HybridTrendingCarousel Component
 * Displays trending recipes blending internal and external sources
 * Shows alternating pattern: internal, external, internal, external
 * Includes trending indicators and source badges
 */
export function HybridTrendingCarousel({ limit = 12, includeWeb = true }) {
  const { data: recipes = [], isLoading } = useHybridTrending(limit, includeWeb);
  ...
  if (!recipes || recipes.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400">No trending recipes found.</p>
      </div>
    );
  }
