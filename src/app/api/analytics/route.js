...
async function getUserAnalytics(supabase, userId) {
  try {
    ...
    try {
      ...
      if (timeError) {
        console.error('Error fetching cooking sessions for time calculation:', timeError)
      } else {
        // Calculate total cooking time in minutes
        totalCookingTime = completedSessions?.reduce((total, session) => {
...
async function getCookingTimeStats(supabase, userId) {
  try {
    ...
    if (data.length > 0) {
      const totalEstimated = data.reduce((sum, session) => sum + (session.estimated_time || 0), 0)
      const totalActual = data.reduce((sum, session) => sum + (session.actual_time || 0), 0)
      ...
      stats.sessions_on_time = data.filter(s => (s.actual_time || 0) <= (s.estimated_time || 0)).length
      stats.accuracy_percentage = Math.round((stats.sessions_on_time / data.length) * 100)
    }
